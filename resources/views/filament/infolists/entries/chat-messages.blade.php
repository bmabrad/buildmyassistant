<div class="space-y-3">
    @foreach ($getRecord()->chats()->orderBy('created_at', 'asc')->get() as $message)
        <div class="rounded-lg p-3 {{ $message->is_deliverable ? 'border-l-4 border-green-500 bg-green-50 dark:bg-green-950' : ($message->role === 'user' ? 'bg-gray-100 dark:bg-gray-800' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700') }}">
            <div class="flex items-center gap-2 mb-1">
                ...
                @if ($message->is_deliverable)
                    <span class="text-xs bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-1.5 py-0.5 rounded">Playbook</span>
                @endif
